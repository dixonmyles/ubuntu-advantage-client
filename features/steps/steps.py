...
@when("I run shell command `{command}` {user_spec} on the `{machine}` machine")
def when_i_run_shell_command_on_machine(context, command, user_spec, machine):
    when_i_run_command(
        context, 'sh -c "{}"'.format(command), user_spec, instance_name=machine
    )
